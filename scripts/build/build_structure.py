...
def feature_counts(features: pl.DataFrame) -> pl.DataFrame:
    return (
        ...
        .agg(
            ...
            n_binding_sites=(pl.col("feature_type") == "binding_site").sum(),
        )
    )
...
def build_structure(models_dir: Path) -> pl.DataFrame:
    ...
    experimental = read_tsv("experimental.tsv", EXPERIMENTAL_SCHEMA)
    ...
    df = (
        ...
        .with_columns(
            pl.col("n_transmembrane", "n_binding_sites", "n_experimental").fill_null(0),
    ...
    return df.with_columns(pl.col("model_file").is_in(present).alias("model_available"))
...
def main() -> None:
    ...
    structure = build_structure(models_dir)
    structure.write_parquet(OUT_DIR / "structure.parquet")
    read_tsv("features.tsv", FEATURE_SCHEMA).write_parquet(OUT_DIR / "features.parquet")
    read_tsv("experimental.tsv", EXPERIMENTAL_SCHEMA).write_parquet(
        OUT_DIR / "experimental.parquet"
    )
