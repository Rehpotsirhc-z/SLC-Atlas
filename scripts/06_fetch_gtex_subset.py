...
"""Subset the full GTEx per-sample gene-TPM matrix to the target genes and capture
...
every other column is one sample's TPM. We strip the version, keep only the target
...
Usage:
    python scripts/06_fetch_gtex_subset.py [genes.parquet] [gtex_tpm.parquet] \\
...
DEFAULT_TPM_OUT = RAW_DIR / "gtex_tpm.parquet"
