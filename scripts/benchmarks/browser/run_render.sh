...
r () { node "$D/render.mjs" --reps="$REPS" --stable=1200 --min=2000 --timeout=60000 --out="$OUT" "$@"; }
for entry in "${LOCI[@]}"; do
  IFS='|' read -r bucket symbol locus <<< "$entry"
  echo "== $bucket $locus" >&2
  r --tool=igv --label='igv.js' --bucket="$bucket" --locus="$locus" \
    --url="http://127.0.0.1:8080/igv_full.html?loc=$locus&set=full"
  r --tool=jbrowse --label='JBrowse 2' --bucket="$bucket" --locus="$locus" \
    --url="http://127.0.0.1:8080/jbrowse-web/?config=/config_full.json&assembly=hg38&loc=$locus&tracks=$JBFULL"
  r --tool=atlasforge_nginx --label='AtlasForge (nginx)' --bucket="$bucket" --locus="$locus" \
    --url="http://127.0.0.1:8080/browser/?gene=$symbol&loc=$locus"
  r --tool=atlasforge_serve --label='AtlasForge (atlas serve)' --bucket="$bucket" --locus="$locus" \
