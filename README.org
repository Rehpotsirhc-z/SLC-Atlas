#+title: SLC Atlas

Turn a gene family into a self-hosted web atlas for exploring annotation,
expression, conservation, sequence similarity, and protein structure. SLC Atlas
gathers public or user-provided data into editable source files, builds a site
from them, and serves it locally or exports it for any static web host.

* Requirements
- Python 3.12 or newer
- MAFFT for the Clustering view
- Node.js only when developing the frontend from a checkout
- Docker and the Dev Container CLI for the recommended development setup

The Structure view is optional. MAFFT is also optional when both ~atlas fetch~ and
~atlas build~ use ~--skip-clustering~.

* TODO Quick start
Install the complete data pipeline:

#+begin_src bash
pip install 'slc-atlas[pipeline]'
#+end_src

Then fetch, build, and serve a family:

#+begin_src bash
atlas fetch 752 --data-dir ./my-atlas
atlas build --data-dir ./my-atlas
atlas serve --data-dir ./my-atlas
#+end_src

Open http://127.0.0.1:8000. Here, ~752~ is an HGNC gene group ID. You can instead
pass a text file containing one gene symbol or Ensembl gene ID per line, or a
downloaded HGNC group TSV. Run ~atlas fetch~ without a source for an interactive
setup.

The first fetch creates curation files and then pauses for user inspection.
Review the family names, excluded genes, display symbols, species, and UniProt
mappings in ~my-atlas/curation/~, and then run the command printed in the
terminal. Existing files are never overwritten, so rerunning a fetch resumes the
dataset and keeps your edits.

* Available Views
- Genes :: Gene annotation and expandable transcript details
- Clustering :: Protein sequence, coding sequence, and co-expression similarity
- Conservation :: Orthologs across species alongside their evolutionary tree
- Expression :: Tissue expression with anatomy diagrams linked to the heatmap
- Genome Browser :: Genomic context, surrounding regions, and annotation tracks
- Structure :: Predicted and experimental models with membrane topology and residue-level confidence

* Fetch and build options
Use ~atlas fetch --help~ and ~atlas build --help~ for the complete command
reference. The options most often used when creating a dataset are:

| Option | Purpose |
|--------+---------|
| ~--curation-dir PATH~ | Store editable decisions outside the data directory |
| ~--gtex-version v8, v10, or v11~ | Choose the GTEx release; the default is v11 |
| ~--gtex-file PATH~ | Use a local ~.gct.gz~ or Parquet expression matrix |
| ~--ensembl-release N~ | Pin the Ensembl release; the default is 116 |
| ~--tree-source NAME~ | Use ~ensembl_compara~, ~ncbi~, ~timetree~, or ~ucsc~ |
| ~--promote-alias-prefix PREFIX~ | Prefer matching aliases as display symbols |
| ~--download-predicted~ | Keep local copies of AlphaFold models |
| ~--download-experimental~ | Keep local copies of PDB structures |
| ~--accept-seeds~ | Continue without pausing after new curation files are created |
| ~--skip-VIEW~ | Omit clustering, conservation, expression, or structure |
| ~--step NAME~ | Run only one named step; repeat to select several |
| ~--mafft PATH~ | Use a specific MAFFT executable during the build |

Downloaded data lives in ~source/~, personal decisions in ~curation/~, reusable
downloads in ~cache/~, and the finished dataset in ~app/~. See [[file:docs/source-files.org][Source files]] if you
want to edit, replace, or generate those inputs yourself.

* Naming and configuration
Each setting can be supplied as an ~ATLAS_*~ environment variable, in a ~.env~ file,
or as a flag on ~atlas serve~ and ~atlas export~. A flag takes priority over the
environment, which takes priority over ~.env~.

| Variable | Flag | Purpose |
|----------+------+---------|
| ~ATLAS_DATA_DIR~ | ~--data-dir~ | Dataset containing ~curation/~, ~source/~, ~cache/~, and ~app/~ |
| ~ATLAS_WEB_DIR~ | ~--web-dir~ | Built frontend files to serve |
| ~ATLAS_APP_NAME~ | ~--app-name~ | Full name used by the page and web app manifest |
| ~ATLAS_APP_SHORT_NAME~ | ~--app-short-name~ | Short name used in the app bar |
| ~ATLAS_APP_DESCRIPTION~ | ~--app-description~ | Description shown on the Genes view and in page metadata |
| ~ATLAS_FAMILY_LABEL~ | ~--family-label~ | Singular name used for a family member |
| ~ATLAS_DOWNLOAD_PREFIX~ | ~--download-prefix~ | Prefix for downloaded figures, tables, and trees |
| ~ATLAS_CORS_ORIGINS~ | ~--cors-origins~ | Comma-separated origins allowed to call the API |
| ~ATLAS_HOST~ | ~--host~ | Address used by ~atlas serve~ |
| ~ATLAS_PORT~ | ~--port~ | Port used by ~atlas serve~ |

Naming changes take effect when the site restarts or is exported again. The dataset does
not need to be rebuilt.

* Deployment
While ~atlas serve~ is convenient for local use, a public atlas should run through Docker
or a static web server for reliable restarts, HTTPS, and normal production traffic.

** Docker
Build the dataset first, then point the ~./data~ volume in ~docker-compose.yml~ at
its directory and set the ~ATLAS_*~ names for your atlas:

#+begin_src bash
docker compose up --build
#+end_src

Open http://localhost:8080. The container exports the site when it starts and
serves it through nginx. Put a reverse proxy in front of it when you need HTTPS;
see the [[https://docs.nginx.com/nginx/admin-guide/web-server/reverse-proxy/][nginx guide]] or [[https://caddyserver.com/docs/quick-starts/reverse-proxy][Caddy guide]] for more details on reverse proxying.

** Static hosting
Export the whole atlas when you already have a web server or want a portable snapshot:

#+begin_src bash
atlas export ./site --data-dir ./my-atlas
rsync -a ./site/ user@host:/var/www/atlas/
#+end_src

The output works with nginx, Apache, Caddy, S3, GitHub Pages, and other static
hosts as long as it supports byte-range requests. It must also serve ~404.html~
for unknown page routes while leaving missing files under ~/api/~ as real 404
responses. You can look at [[file:deploy/nginx.conf][deploy/nginx.conf]] for a complete example.
Re-export after changing the dataset or the site name.

* TODO Development
The dev container is the easiest way to setup the project with Python, Node.js,
MAFFT, nginx, and the project dependencies bundled. Install Docker and the Dev
Container CLI, and then run:

#+begin_src bash
./enter-dev.sh
./dev-zellij.sh
#+end_src

The frontend runs at http://localhost:3000 and sends API requests to FastAPI at
http://localhost:8000. A production-style static preview is available at
http://localhost. See [[file:.devcontainer/README.md][Dev container setup]] for direct CLI commands and container details.

For a manual setup:

#+begin_src bash
python -m venv .venv
source .venv/bin/activate
pip install -e '.[pipeline,dev]'
npm --prefix web ci
fastapi dev src/slc_atlas/main.py --host 0.0.0.0 --port 8000
npm --prefix web run dev
#+end_src

Run the two development servers in separate terminals. Before submitting a change, run
~black --check src scripts~, ~npm --prefix web run format:check~, and
~npm --prefix web run build~.

* Licensing
This project is licensed under the Apache-2.0 license and is fully [[https://reuse.software/][REUSE-compliant]].
Anatomogram artwork from EMBL-EBI is available under CC BY 4.0; see [[file:web/src/views/Expression/Anatomograms/anatomogram/ATTRIBUTION.md][Anatomogram
artwork]] for its source, attribution, and modification details.
